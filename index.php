...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($store['accounts'] as $a) {
        if (strcasecmp($a['username'], $username) === 0 && $a['password'] === $password) { $match = $a; break; }
    }
    if ($match) {
        $_SESSION['user'] = $match;
        header('Location: dashboard.php');
    ...
    $error = "We couldn't verify those credentials. Please check your username and password.";
}
?>
<!doctype html>
<html lang="en">
...
<div class="auth-shell">
  <div class="auth-card">
    <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:26px;">
      ...
      <div class="display" style="font-size:19px;font-weight:700; text-align:center;">TazamaDesk</div>
    </div>

    <div class="auth-box">
      <div style="margin-bottom:20px;">
        <div class="display" style="font-size:19px;font-weight:700;margin-bottom:5px; text-align:center;">Log in to your account</div>
       <!-- 
      ...
      <form method="post" action="index.php" style="display:flex;flex-direction:column;gap:14px;">
        <div>
          ...
          <input type="username" name="username" placeholder="Enter your username" value="<?= h($_POST['username'] ?? '') ?>" required>
        </div>
        <div>
